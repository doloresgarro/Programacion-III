...
public class ListaDeEnterosEnlazada extends ListaDeEnteros {
	...
	public int BuscarPos(int aux, ListaDeEnterosEnlazada l) {
		l.comenzar();
		int pos = 0;
		while ((! l.fin()) && (aux < l.proximo())) {
			pos++;
		}
		return pos;
	}
	...
	public ListaDeEnterosEnlazada ordenar (ListaDeEnterosEnlazada l) {
		...
		listaOrdenada.agregarInicio(l.proximo()); // el primer elemen lo agrego
		int pos = 0, aux;
		l.comenzar();
		while (! l.fin()) {
			aux = l.proximo(); 
			
			pos = BuscarPos(aux, listaOrdenada);
			listaOrdenada.agregarEn(aux, pos);
